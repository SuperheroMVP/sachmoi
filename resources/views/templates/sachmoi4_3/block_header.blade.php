...
<header class="header header-1">
    ...
    <div class="header-middle header-1--middle brand-bg-2 d-flex align-items-center">
        <div class="container">
            <div class="row align-items-center">
                <div class="col-xl-2 tex-xl-left text-center">
                    <!-- Logo Start -->
                        ...
                        <ul class="mainmenu">
                            ...
                            @if (!empty(sc_config('Content')))
                                ...
                                @if(count($cmsCategories)>0)
                                    <li class="mainmenu__item menu-item-has-children">
                                        <a class="mainmenu__link" href="{{route('cms.news.all')}}">{{ trans('front.blog') }}</a>
                                            @if (count($cmsCategories)>0)
                                                <ul class="sub-menu">
                                                    @foreach ($cmsCategories as $key => $cms_ategories)
                                                      <li><a href="{{ $cms_ategories->getUrl() }}">{{ $cms_ategories->title }}</a></li>
                                                    @endforeach
                                                </ul>
                                            @endif
                                    </li>
...
{{--                    {{dd(URL::current())}}--}}
                    <div class="category-nav">
                        <h2 class="category-nav__title tertiary-bg" id="js-cat-nav-title"><i class="fa fa-bars"></i> <span>{{trans('front.categories')}}</span></h2>

                        @if ($categoriesTop->count())
                            <ul class="category-nav__menu
                                @if(URL::current()!="http://sachmoi.vn"&& URL::current()!="http://sachmoi.vn/public"&& URL::current()!="https://sachmoi.vn"&& URL::current()!="https://sachmoi.vn/public")
                                hide-in-default"
                                @endif
                                id="js-cat-nav">
...
{{--                                         <li class="category-nav__menu__item has-children">
                                      ...
                                      <li class="category-nav__menu__item has-children">
                                        ...
                                        <div class="category-nav__submenu mega-menu three-column">
                                            @foreach ($categories[$category->id] as $cateChild)
                                            <div class="category-nav__submenu--inner">
                                                <a href="{{ $cateChild->getUrl() }}"><h3 class="category-nav__submenu__title">{{ $cateChild->title }}</h3></a>
                                                <ul>
                                                    @if(!empty($categories[$cateChild->id]))
                                                        @foreach ($categories[$cateChild->id] as $cateChild2)
                                                        <li><a href="{{ $cateChild2->getUrl() }}">{{ $cateChild2->title }}</a></li>
                                                        @endforeach
                                                    @endif
                                                </ul>
                                            </div>
            ...
            <div class="row">
                <div class="col-lg-3">
                    ...
                    <a class="sticky-logo" href="{{asset('')}}">
                        <img src="{{ asset(sc_store('logo')) }}" width="120px" alt="logo">
                    </a>
                    <!-- Sticky Logo End -->
                </div>
                <div class="col-lg-9">
