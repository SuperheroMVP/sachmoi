...
if (!function_exists('sc_percent_render')) {
    /**
     * Render currency: format number, change amount, add symbol
     *
     * @param   float  $percent                 [$money description]
     */
    function sc_percent_render(float $price ,float $priceFinal)
    {
        $percent = ($priceFinal - $price)/$price *100;
        return ShopCurrency::sc_percent_render_main($percent);
    }
}
